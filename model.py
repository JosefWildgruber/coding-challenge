...
class TermDepositClassifier:
    ...
    def _balance_dataset(
        self, X: pd.DataFrame, y: pd.DataFrame, mode="smote"
    ...
    def fit(self, cv=3, n_jobs=3, verbose=1, scoring=None, refit=False):
        self.data = self._drop_unknown(
            data=self.data, col_to_clean=columns_to_clean
        ...
        self.X_train = self._get_dummies(
            X=self.X_train, numerical_columns=self.numerical_columns
        )
        print("Create One-Hot Encodings: Done")
        pd.DataFrame({"Columns": self.X_train.columns}).to_csv(
            f"{model_path}expected_columns.csv"
        )
        print("Save list of expected columns: Done")
        self.X_train, self.y_train = self._balance_dataset(
            X=self.X_train, y=self.y_train, mode="smote"
        )
    ...
    def predict(self):
        ...
        self.X_test = self._get_dummies(
            X=self.X_test, numerical_columns=self.numerical_columns
        )
        print("Create One-Hot Encodings: Done")
        ...
        self.prediction_results = pd.concat(
            [self.data, self.prediction_results], axis=1
        )
    ...
    def get_score(self):
        ...
        for item in self.grid_searches.cv_results_["params"]:
            ...
            d["mean_test_f1_score"].append(
                self.grid_searches.cv_results_["mean_test_f1"][index]
            )
            d["mean_test_accuracy"].append(
                self.grid_searches.cv_results_["mean_test_accuracy"][index]
            )
            d["mean_test_recall"].append(
                self.grid_searches.cv_results_["mean_test_recall"][index]
            )
